...
// NOTE: On Vercel, this in-memory data will reset whenever the serverless function goes to sleep.
...
app.post('/api/admin/reboot', requireAdmin, (req, res) => {
    ...
    // Only exit if not running in Vercel's serverless environment
...
// ========== VERCEL & LOCAL STARTUP EXPORT ==========

if (process.env.NODE_ENV !== 'production') {
    const PORT = process.env.PORT || 3000;
    app.listen(PORT, () => {
        console.log(`🚀 REAL Sparky Backend Server is running locally on http://localhost:${PORT}`);
    });
}
